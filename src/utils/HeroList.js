...
/**
 * 获取科技类型
 * @param {number} techId - 科技ID
 * @returns {number} 科技类型
 */
export const getTechType = (techId) => {
  if (techId >= 1 && techId <= 10) return 1;
  if (techId >= 11 && techId <= 20) return 2;
  if (techId >= 21 && techId <= 30) return 3;
  if (techId >= 31 && techId <= 40) return 4;
  return 0;
};
