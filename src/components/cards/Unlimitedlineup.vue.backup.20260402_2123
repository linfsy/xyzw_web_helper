...
const exportLineups = async () => {
  ...
  const tokenId = token.id;
  const roleId = token.id;
  const roleName = token.name || "未知角色";
  const server = token.server || "未知服务器";
  ...
  const fileName = `阵容配置_${roleName}_${server}_${new Date().toLocaleDateString().replace(/\//g, "-")}.json`;
  ...
  if (isAndroidApp) {
    try {
      ...
      message.success(`阵容配置已导出到: /storage/emulated/0/Documents/${fileName}`);
    } catch (error) {
  ...
  reader.onload = (e) => {
    try {
      ...
      if (!importData.roleId || !importData.lineups) {
        message.error("无效的阵容配置文件格式");
        return;
      }

      if (!Array.isArray(importData.lineups)) {
        message.error("阵容配置格式错误");
        return;
      }

      // 使用naive-ui的dialog替代confirm
      dialog.warning({
        title: '确认导入',
        content: `确定要导入 ${importData.lineups.length} 个阵容配置吗？`,
        positiveText: '确定',
        negativeText: '取消',
        onPositiveClick: () => {
          savedLineups.value = importData.lineups;
          saveLineupsToStorage();
          message.success(`成功导入 ${importData.lineups.length} 个阵容配置`);
        }
      });
    } catch (error) {
